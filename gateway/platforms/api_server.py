...
def _chat_usage_payload(usage: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = dict(zip(("prompt_tokens", "completion_tokens", "total_tokens"), values))
    if isinstance(usage, dict):
        actual_runtime = usage.get("runtime")
        actual_model = (
            actual_runtime.get("model")
            if isinstance(actual_runtime, dict) and actual_runtime.get("model")
            else usage.get("used_model")
        )
        if actual_model:
            payload["used_model"] = actual_model
        if actual_runtime:
            payload["runtime"] = actual_runtime
    return payload
