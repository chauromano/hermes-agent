...
def _resolve_region(explicit: Optional[str] = None) -> str:
    ...
    if explicit:
        return explicit
    for env_var, cfg_key in (("VERTEX_REGION", "region"), ("VERTEX_LOCATION", "location")):
        val = _env_or_config(env_var, cfg_key)
        if val:
            return val
...
def _resolve_project_override() -> Optional[str]:
    """Project-ID override (VERTEX_PROJECT_ID / VERTEX_PROJECT / GOOGLE_CLOUD_PROJECT env > config.yaml), or None to use the creds' embedded project_id."""
    for env_var, cfg_key in (("VERTEX_PROJECT_ID", "project_id"), ("VERTEX_PROJECT", "project"), ("GOOGLE_CLOUD_PROJECT", "project")):
        val = _env_or_config(env_var, cfg_key)
        if val:
            return val
    return None
...
def has_vertex_credentials() -> bool:
    ...
    if _resolve_credentials_path(None) or _resolve_project_override():
        return True
